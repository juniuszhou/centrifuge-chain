...
macro_rules! new_full_start {
    ($config:expr) => {{
        ...
        .with_rpc_extensions(
            |client, pool, _backend, _fetcher, _remote_blockchain| -> Result<RpcExtension, _> {
        ...
        ServiceBuilder::new_light::<Block, RuntimeApi, crate::executor::Executor>(config)?
            ...
            .with_rpc_extensions(
                |client, pool, _backend, fetcher, remote_blockchain| -> Result<RpcExtension, _> {
                    ...
                    let remote_blockchain = remote_blockchain.ok_or_else(|| {
                        "Trying to start node RPC without active remote blockchain"
                    })?;
